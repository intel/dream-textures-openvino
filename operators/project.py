...
from ..ui.panels.dream_texture import advanced_panel_project, create_panel, prompt_panel, size_panel
from .dream_texture import CancelGenerator, ReleaseGenerator, LoadModel, ProjectLoadModel
...
def dream_texture_projection_panels():
    class DREAM_PT_dream_panel_projection(bpy.types.Panel):
        ...
        def draw(self, context):
           ...
           # if Pipeline[context.scene.dream_textures_project_prompt.pipeline].model():
            ...
            layout.prop(context.scene.dream_textures_project_prompt, "infer_model")
            layout.prop(context.scene.dream_textures_project_prompt, "infer_model_size")
            #row = layout.row()
    ...
    yield from create_panel('VIEW_3D', 'UI', DREAM_PT_dream_panel_projection.bl_idname, advanced_panel_project, get_prompt)
            ...
            def draw(self, context):
                ...
                if context.scene.dream_textures_progress <= 0:
                    if context.scene.dream_textures_info != "":
                        ...
                        #r = row.row()
                        row.operator(ProjectLoadModel.bl_idname, icon="PLAY", text="Load Models Project")
                        row.operator(ReleaseGenerator.bl_idname, icon="X", text="")
                        row.operator(ProjectDreamTexture.bl_idname, icon="MOD_UVPROJECT")
                        
                        row.enabled = Pipeline[context.scene.dream_textures_project_prompt.pipeline].depth() and context.object is not None and context.object.mode == 'EDIT'
                else:
                ...
                try:
                    ...
                    if generated_args["infer_model"].name == "Stable_Diffusion_1_5_controlnet_depth_int8":
                        ...
                        prompt.validate(context,task=ModelType.DEPTH_INT8)
...
#class ReleaseGeneratorProject(bpy.types.Operator):
#    bl_idname = "shade.dream_textures_release_generator_project"
#    bl_label = "Release Generator Project"
#    bl_description = "Releases the generator class to free up VRAM"
#    bl_options = {'REGISTER'}

#    def execute(self, context):
#        kill_generator(context)
#        ProjectLoadModel.Models_loaded_depth_project = False
...
class ProjectDreamTexture(bpy.types.Operator):
    ...
    def poll(cls, context):
        generated_args = context.scene.dream_textures_project_prompt.generate_args()
        #print("------ Outside if statements CHECK In generate project Models_loaded_depth_512----",  generated_args['infer_model_size'].name)
        #print("A.  ProjectLoadModel.Models_loaded_depth_project in project---", ProjectLoadModel.Models_loaded_depth_project)
        #print("B. LoadModel.Models_loaded_depth in project---", LoadModel.Models_loaded_depth)
        try:
             
            if generated_args["infer_model"].name == "Stable_Diffusion_1_5_controlnet_depth": # and ProjectLoadModel.Models_loaded_depth_project== True: #or LoadModel.Models_loaded_depth == True:
    ...
    def execute(self, context):
        ...
        def on_exception(_, exception):
            ...
            raise exception
        #print("PROCESS ID in Depth", os.getpid())
        #generated_args = scene.dream_textures_prompt.generate_args()

        #supported_model = generated_args['infer_model'].name
        #if  supported_model != "Stable_Diffusion_2_1_depth":
        #    raise ValueError("Selected model does not support depth conditioning. Please select a different model, such as 'Stable_Diffusion_2_1_depth' or change the 'Image Type' to 'Color'.")
        #else:
        context.scene.dream_textures_info = "Starting..."
        ...
        if supported_model == "Stable_Diffusion_1_5_controlnet_depth_int8": 
            future = gen.depth_to_image_int8(
                depth=depth,
                image=init_img_path,

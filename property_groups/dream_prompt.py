...
from ..generator_process.actions.prompt_to_image import Optimizations, Scheduler, StepPreviewMode, Pipeline, device_name_enum, model_name_enum, model_size_enum, device_name_enum_npu
...
#scheduler_options = [Scheduler.EULER_DISCRETE, Scheduler.LMS_DISCRETE, Scheduler.DPM_SOLVER_MULTISTEP]
scheduler_options = [(scheduler.value, scheduler.value, '') for scheduler in Scheduler]

device_options = [(infer_device.value, infer_device.value, '') for infer_device in device_name_enum]
device_options_npu = [(infer_device.value, infer_device.value, '') for infer_device in device_name_enum_npu]

ov_model_options = [(infer_model.value, infer_model.value, '') for infer_model in model_name_enum]

ov_model_size_options = [(infer_model_size.value, infer_model_size.value, '') for infer_model_size in model_size_enum]
...
attributes = {
    "pipeline": EnumProperty(name="Pipeline", items=pipeline_options, default=1, description="Specify which model and target should be used."),

    "infer_model": EnumProperty(name="OpenVINO Model", items=ov_model_options, description="Specify which openvino model to use for inference", default=1),
    "infer_model_size": EnumProperty(name="Model Size", items=ov_model_size_options, description="Specify the size of the model to be used", default=0),
   ...
   # "seamless_axes": EnumProperty(name="Seamless Axes", items=seamless_axes, default=SeamlessAxes.AUTO.id, description="Specify which axes should be seamless/tilable"),
    ...
    "scheduler": EnumProperty(name="Scheduler", items=scheduler_options, default=2), # defaults to "DPM Solver Multistep"
    "infer_device_text": EnumProperty(name="Text_Encoder_Device", items=device_options, default=1),
    "infer_device_unet": EnumProperty(name="Unet_Device", items=device_options, default=1),
    "infer_device_unet_pos": EnumProperty(name="Unet_Pos_Device", items=device_options_npu, default=0),
    "infer_device_unet_neg": EnumProperty(name="Unet_Neg_Device", items=device_options_npu, default=0),
    "infer_device_vae": EnumProperty(name="Vae_Device", items=device_options, default=1),
...
def generate_args(self):
    ...
    args['infer_device_text'] = device_name_enum(args['infer_device_text'])
    args['infer_device_unet'] = device_name_enum(args['infer_device_unet'])
    try:
        args['infer_device_unet_pos'] = device_name_enum_npu(args['infer_device_unet_pos'])
        args['infer_device_unet_neg'] = device_name_enum_npu(args['infer_device_unet_neg'])
    except:
        print("unet pos, neg device not set--check if you have NPU and correct openvino env")
    args['infer_device_vae'] = device_name_enum(args['infer_device_vae'])
    args['infer_model'] = model_name_enum(args['infer_model'])
    args['infer_model_size'] = model_size_enum(args['infer_model_size'])
...
weight_path = os.path.join(os.path.expanduser("~"), "Blender_SD_models")
DreamPrompt.weight_path = weight_path
